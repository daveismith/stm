...
export const onSceneReady = (scene: Scene) => {
    ...
    const tableRadius = 5;
    const tableHeight = 9;
    var playerSettings: Vector3[] = [];
    const players = 6;
    const deck: Mesh[] = [];
    const deckPosition = new Vector3(1.5, tableHeight + 0.01, -1.5);
    const deckSize = 48;
    const playMatPositions: Vector3[] = [];
    const handPositions: Vector3[] = [];
        ...
        for (var i = 0; i < players; i++) {
            playerSettings.push(
                new Vector3(
                    tableRadius *
                    settingRatio *
                    Math.sin((2 / players) * Math.PI * i),
                    tableHeight + 0.01,
                    tableRadius *
                    settingRatio *
                    Math.cos((2 / players) * Math.PI * i)
                )
            );
            playMatPositions.push(
                new Vector3(
                    tableRadius *
                    playMatRatio *
                    Math.sin((2 / players) * Math.PI * i),
    ...
    const animateCardSlide = (
        ...
        keyFramesPY.push({
            frame: (frameRate + frameRate * frameOffset) / cardsPerSecond,
            value:
                targetPosition.y + Math.floor(frameOffset / players) * 0.0072,
        });
        ...
        scene.beginDirectAnimation(
            ...
            0,
            (frameRate + frameRate * frameOffset) / cardsPerSecond,
            true
        );
    };
    ...
    const playCard = (card: Mesh, player: number) => {
    ...
    const animateAddCardToFan = (
        ...
        keyFramesPX.push({
            frame: frameRate / cardsPerSecond,
            value:
                handPositions[player].x +
                ((fanPosition - deckSize / players / 2) * handRadius.x) /
                (deckSize / players),
        });
        ...
        keyFramesPY.push({
            frame: frameRate / cardsPerSecond,
            value:
                handPositions[player].y +
                (0.016 * deckSize) / players -
        ...
        keyFramesPZ.push({
            frame: frameRate / cardsPerSecond,
            value:
                handPositions[player].z +
                ((-1 / 64) *
                    Math.pow(fanPosition + 0.5 - deckSize / players / 2, 2) +
                    0.5) *
                handRadius.z,
        });
        ...
        keyFramesRY.push({
            ...
            value:
                ((fanPosition + 0.5 - deckSize / players / 2) * Math.PI) / 16,
        });
